...
export default {
  ...
  methods: {
    ...
    async deleted() {
      let data = this.itemsCustomer.map(item => item.id)
    ...
    templateName(dataItem) {
      return (
        ...
        kendo.htmlEncode(dataItem.business_name) +
        "</span>"
      )
    },
    ...
    onChange(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsCustomer.findIndex(x => x.id == idValue)
        if (row.className.indexOf("k-state-selected") >= 0) {
          let {parent, dirty, dirtyFields, _events, _handlers, uid,...obj} = grid.dataItem(row)
          vm.$store.dispatch("setItemCustomer", obj)
        } else if (index > -1) {
          vm.$store.dispatch("deleteItemCustomer", index)
        }
      })
    },
    ...
    dataBound(e) {
      ...
      items.each(function(idx, row) {
        ...
        let index = vm.itemsCustomer.some(item => item.id === dataItem)
        if (index) {
    ...
    async pdfListCustomer() {
      this.$store.dispatch("showLoader")
      let data = this.itemsCustomer.map(item => item.id)
      try {
        const customers = await CustomerService.pdfListCustomer({customer: data})
        if (customers.status === 200) {
          let blob = new Blob([customers.data])
    ...
    async excelListCustomer() {
      this.$store.dispatch("showLoader")
      let data = this.itemsCustomer.map(item => item.id)
      try {
        const customers = await CustomerService.excelListCustomer({customer: data})
        if (customers.status === 200) {
          let blob = new Blob([customers.data])
          let link = document.createElement("a")
          link.href = window.URL.createObjectURL(blob)
