...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridProduct.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridProduct.kendoWidget().dataSource.filter({})
    },

    update(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridProduct.kendoWidget()
    ...
    async deleted() {
      ...
      if (response.status === 200) {
        ...
        this.$refs.gridProduct.kendoWidget().dataSource.read()
