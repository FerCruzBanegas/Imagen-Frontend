...
export default {
  ...
  mounted() {
    let grid = this.$refs.gridExecuted.kendoWidget()
    let ds = []
  ...
  methods: {
    reloadTable() {
      this.$refs.gridExecuted.kendoWidget().dataSource.filter({})
    },
    ...
    setVoucher(ev) {
      ev.preventDefault()
      let gridWidget = this.$refs.gridExecuted.kendoWidget()
    ...
    async submit() {
      ...
      try {
        ...
        if (this._save.status === 201) {
          this.$refs.gridExecuted.kendoWidget().dataSource.read()
          this.visibleListInvoice = false
          this.closeModalForm()
